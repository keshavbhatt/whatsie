...
- **Chromium sandbox stays on** (ADR-008): we never pass `--no-sandbox`; the `browser-sandbox`
  plug (`browser-support` with `allow-sandbox: true`) lets Chromium create its user namespaces.
  Store auto-connect for that attribute needs a snap declaration (a forum store-request); a local
  install works with `snap connect whatsie:browser-sandbox`.
