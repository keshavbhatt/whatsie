...
namespace Identicons {
  ...
  static const size_t tileColorCount = sizeof(letterTileColors) / sizeof(QRgb);
  ...
  QPixmap letterTile(const QString & text, const QSize & size) {
    ...
    if (trimmed.length() == 0) return QPixmap();
    QString s;
    QStringList l = trimmed.split(" ", Qt::SkipEmptyParts);
    s += l.first().front();
    if(l.length() > 3)
    ...
    qint64 idx = static_cast<qint64>(qHash(trimmed)) - 16;
    idx = ((idx < 0)?-idx:idx) % tileColorCount;
